...
void impBarco( char tabJug[tamTab][tamTab][2], char tabMaq[tamTab][tamTab][2], int longBar, int simAUsar, int tipoJugador )
{
	...
	*/
	...
	if( tipoJugador == 0 )
	{
		...
		// Pedimos la coordenada de inicio
		
		printf( "Dame la coordenada de inicio del barco, ejemplo: 1a\f" );
		do
		{
			scanf( "%s", coor );
		} while( validaYSeparaFilyCol( tabJug, coor, &fil, &col, tipoJugador ) != 1 );

		tabJug[fil][col][0] = simbolosBarcos[simAUsar];										//Pone en el tablero el inicio del barco:
		impMovPosBarco( tabJug, fil, col, longBar );										//Muestra las posiciones disponibles para el barco atendiendo al tamaño:
		impTablero(tabJug);																	//Muestra el tablero:
		impFinBarco( tabJug, fil, col, longBar, simbolosBarcos[simAUsar], tipoJugador );	//Muestra el final y resto del barco:
		borrMovPosBarco( tabJug );															//Borra las posiciones no usadas:

	}
	else // El ordenador genera sus coordenadas:
	{
		do
		{
			//Generación de coordenadas aleatoria, si ya están ocupadas se generan otras hasta conseguir unas validas.
			...
			coor[0] = (rand()%tamTab)+48;		//Numero del 0 al tamTab al cual se le suman 48 para obtener el numero ASCII correspondiente.
			coor[1] = (rand()%tamTab)+48+49;	//Numero del 0 a tamTab, que posteriormente se le suman 48 para tener el numero en ASCII y +49 para el correspondiente en letra.
